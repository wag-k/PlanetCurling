...
PlanetCurlingは、惑星同士の万有引力を使ったカーリング風ゲームを目指すAkashic Engine製のプロトタイプです。現在はPhase G1として、同じ画面でRedとBlueが交互に投球するローカル2人対戦を実装しています。
...
- 投球後に5ゲーム年だけ進み、照準中は停止する物理シミュレーション
- 投球済み惑星を盤面とPhysicsWorldへ残す多体重力
- 現在ターン・投球数・状態表示とNew Game
- 6時間の固定物理タイムステップ
- Symplectic Euler / Velocity Verletの切り替え
- Akashic Engineに依存しない物理層の単体テスト

得点、勝敗、ターゲット軌道、衝突、軌道予測、軌跡、CPU、ネット対戦などはまだ実装していません。

## 遊び方

1. 画面上の現在プレイヤー（Redは`planet1`、Blueは`planet2`の画像）をドラッグします。
2. ドラッグを離すと投球が確定し、物理世界が5ゲーム年進みます。
3. 盤面が停止したら次のプレイヤーが投球します。
4. RedとBlueが3投ずつ終えると`Match Finished`になります。
5. 右上の **New Game** で中央天体を含む盤面と投球数を初期化できます。

照準中は物理時間が停止するため、過去の惑星は動きません。投球済み惑星は消えず、中央天体や後続の投球惑星と同じNewton重力の重力源になります。過去の惑星を再度ドラッグすることはできません。
...
- `Setting.SimulationDurationPerShotSeconds`: 1投後に進める時間。365日基準の5年
...
暫定ゲームバランス値は`src/game_balance.ts`へ集約しています。中央天体は`6 × 10^26 kg`、投球惑星は`6 × 10^24 kg`（中央天体の1%）です。現実の小天体より意図的に大きくし、近づいた投球惑星同士が後続軌道へ目に見える影響を与えつつ、中央天体を主な軌道形成要因にしています。万有引力定数やNewton重力式は変更していません。
...
- 衝突判定と重力ソフトニングは未実装です。
- 得点、勝敗、ターゲット軌道は未実装のため、6投後は結果表示だけで終了します。
- 投球惑星の質量と発射位置はG1用の暫定値であり、プレイテスト後の調整が必要です。
