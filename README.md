...
PlanetCurlingは、惑星同士の万有引力を使ったカーリング風ゲームを目指すAkashic Engine製のプロトタイプです。現在はPhase G2として、ローカル2人対戦にターゲット軌道、暫定得点、勝敗を追加しています。
...
衝突、軌道予測、軌跡、CPU、ネット対戦などはまだ実装していません。

## 遊び方

1. 画面上の現在プレイヤー（Redは`planet1`、Blueは`planet2`の画像）をドラッグします。
2. 点線リングを位置の目安にドラッグを離すと投球が確定し、物理世界が10ゲーム年進みます。
3. 盤面が停止したら次のプレイヤーが投球します。
4. リリース済み惑星は、中心天体との距離と相対動径速度から暫定採点されます。
5. RedとBlueが3投ずつ終えると得点を確定し、勝敗を表示します。
6. 右上の **New Game** で中央天体、投球、得点、勝敗を初期化できます。
